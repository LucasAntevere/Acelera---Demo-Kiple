...
export default function DepartmentsPage() {
  ...
  const save = async () => {
    ...
    if (!payload.nome) {
      setErrorMsg("Nome do departamento é obrigatório.");
      return;
    }
  ...
  return (
    <DashboardLayout title="Departamentos" subtitle="Gestão de departamentos organizacionais">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-xl font-bold text-foreground flex items-center gap-2">
            Departamentos
            {loading && <Loader2 className="h-4 w-4 animate-spin text-muted-foreground" aria-label="Carregando departamentos" />}
          </h2>
          <p className="text-sm text-muted-foreground mt-0.5">{departamentos.length} departamentos cadastrados</p>
        </div>
        <Button size="sm" className="h-8 text-xs gap-1.5" onClick={openCreate} title="Cadastrar novo departamento" aria-label="Cadastrar novo departamento">
          <Plus className="h-3.5 w-3.5" aria-hidden="true" /> Novo Departamento
        </Button>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl p-4 card-shadow mb-4">
        ...
        <div className="relative mt-1.5">
          ...
          <Input
            ...
            title="Busca por nome do departamento"
            aria-label="Busca por nome do departamento"
      ...
      {formOpen && (
        <div className="bg-card border border-border rounded-xl p-4 card-shadow mb-4 space-y-3">
          ...
          <div>
            <Label htmlFor="departamento-nome" className="text-xs text-muted-foreground">Nome do departamento</Label>
            <Input
          ...
          <div className="flex gap-2 pt-1">
            <Button size="sm" className="h-8 text-xs" onClick={() => void save()} title="Salvar departamento">Salvar</Button>
      ...
      <div className="bg-card border border-border rounded-xl card-shadow overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-xs" aria-label="Tabela de departamentos">
            <thead>
                ...
                <tr key={d.id} className={i % 2 === 0 ? "" : "bg-muted/5"}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2 justify-end">
                      <Button size="sm" variant="outline" className="h-7 text-[11px]" onClick={() => openEdit(d)} title={`Editar departamento ${d.nome}`}>
                      ...
                      <Button
                        ...
                        title={`Excluir departamento ${d.nome}`}
                        onClick={async () => {
